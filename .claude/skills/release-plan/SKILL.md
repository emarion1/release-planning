...
description: |
  ...
  - "Which features are blocked or unscheduled?"
...
- data/features-ready.json — sized and ranked RHOAI features
...
and the full features list (sizing, scheduling, ranking).

Read data/schedule.json. Extract: schedule (releases/events), plan
(bucket_key to features/points/capacity_status), and unscheduled list.

Output: "Read [N] features and a schedule covering [start] to [end]."
...
Blocked features: features where blockedBy is non-empty.
  Check if the blocking feature is scheduled before the blocked one.
...
Unscheduled features: from schedule.json unscheduled list.
  Surface the top 10 by rank.
...
Write structured JSON:

  {
    ...
    "summary": { totalFeatures, scheduledFeatures, unscheduledFeatures,
                 blockedFeatures, xlFeatures, autoSizedFeatures, releasesPlanned },
    "capacity": { ... },
    "releases": [
      { "version": "3.5",
        "events": {
    ...
    "unscheduled": [...],
    "risks": { "blockedFeatures": [], "xlFeatures": [], "overCapacityEvents": [] }
  }

Each feature entry: key, summary, points, sizeMethod, rank, scheduledTo, blockedBy.
